...
def load_config():
    ...
    try:
        ...
        logging.error(f"Could not decode JSON from {CONFIG_PATH}. Returning default config.")
...
def load_rotation_status():
    ...
    with open(ROTATION_STATUS_PATH, "r") as f:
        try:
            ...
            logging.error(f"Could not decode JSON from {ROTATION_STATUS_PATH}. Returning empty status.")
