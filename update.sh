...
# Function to clean selected version string
clean_version() {
    echo "$1" | sed 's/\x1B\[[0-9;]*[a-zA-Z]//g' | tr -d '\r\n' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'
}
...
get_versions() {
    ...
    if [[ "$current_branch" == "dev" ]]; then
        echo -e "\e[1;34mShowing last 10 dev versions (tags containing 'dev'):\e[0m" >&2
        mapfile -t versions < <(git -C "$PROGRAM_DIR" tag --sort=-v:refname | grep 'dev' | head -n 10)
    else
        echo -e "\e[1;34mShowing last 10 release versions (tags not containing 'dev'):\e[0m" >&2
        mapfile -t versions < <(git -C "$PROGRAM_DIR" tag --sort=-v:refname | grep -v 'dev' | head -n 10)
    fi

    if [ ${#versions[@]} -eq 0 ]; then
        echo -e "\e[1;33m⚠ No matching tagged versions found.\e[0m" >&2
        echo -e "\e[1;34mFalling back to latest 5 commits on '$current_branch':\e[0m" >&2
        mapfile -t versions < <(git -C "$PROGRAM_DIR" log "$current_branch" --pretty=format:"%h (%s, %ar)" --abbrev-commit --max-count=5)
        if [ ${#versions[@]} -eq 0 ]; then
            echo -e "\e[1;31mError: Could not list any versions or commits.\e[0m" >&2
            echo ""
            return 1
        fi
    fi
    ...
    while true; do
        ...
        if [[ "$user_choice" -eq 0 ]]; then
            ...
            selected_version=$(clean_version "$selected_version")
            echo -e "\e[1;32mSelected: $selected_version\e[0m" >&2
            break
        elif [[ "$user_choice" -eq $(( ${#versions[@]} + 1 )) ]]; then
            read -r -p "Enter version (tag, commit hash, or branch name): " selected_version
            selected_version=$(clean_version "$selected_version")
            if [[ -z "$selected_version" ]]; then
                echo -e "\e[1;31mVersion cannot be empty.\e[0m" >&2
            else
                echo -e "\e[1;32mSelected manually: $selected_version\e[0m" >&2
                break
            fi
        else
...
# Function to update to selected version
update_to_version() {
    local version_to_update_to="$1"
    version_to_update_to=$(clean_version "$version_to_update_to")
